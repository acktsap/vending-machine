...
val targetProjects = rootProject.subprojects
    ...
    // filter out bom project
    .filter { subproject -> !subproject.name.endsWith("bom") }

/*
    This is configuration following official guide.
    But when using with spring dependency management, fails in resolving dependency (without explicit version) in subproject.
    So we don't use it.
 */
// dependencies {
//     targetProjects
